...
def get_points(name):
    ...
    results = [(int(delta*scale), path_y_offset+int(func(delta)*-scale)) for delta in deltas]
...
for idx, easing_func in sv_easing_functions.easing_dict.items():
    ...
    bg = svg.rect(x=0, y=rect_y_offset, width=150, height=150, stroke="#666", fill="none")
